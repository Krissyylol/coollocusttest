...
-- Tries Obsidian (Krissyylol) → Obsidian (deividcomsono) → Manual UI.
--============================================================================--
...
local forceStateChoice = "sprint"
local oldForceStateEnabled = false
...
-- ---- State forcing ----
local function forceStateAlways(state)
    if MovementState and forceSprintEnabled and state then
        pcall(function()
            MovementState:FireServer(state)
...
RunService.Heartbeat:Connect(function()
    if forceSprintEnabled then
        forceStateAlways(forceStateChoice)
    end
end)

-- ---- Old movement-based forcing ----
...
RunService.Heartbeat:Connect(function()
    if oldForceStateEnabled and isMoving and speedEnabled then
        pcall(function() MovementState:FireServer(forcedState) end)
    end
end)
...
-- Attempt to load Obsidian libraries (two attempts)
...
-- Try Krissyylol
if not tryLoadObsidian("https://raw.githubusercontent.com/Krissyylol/Obsidian/main/") then
    -- Try original
    if tryLoadObsidian("https://raw.githubusercontent.com/deividcomsono/Obsidian/main/") then
        useObsidian = true
    end
else
    useObsidian = true
...
-- If Obsidian loaded, build it; else fallback manual UI
if useObsidian and Library then
    -- ==================== OBSIDIAN UI ====================
    ...
    StateGroup:AddToggle("ForceSprintToggle", {
        Text = "Force Sprint (always)",
        Default = false,
        Callback = function(v)
            forceSprintEnabled = v
            if v then
                LocalPlayer:SetAttribute("StaminaVuota", false)
                Library:Notify({ Title = "State", Description = "Forcing " .. forceStateChoice, Duration = 2 })
            else
                Library:Notify({ Title = "State", Description = "State forcing OFF", Duration = 2 })
            end
        end,
    })
    StateGroup:AddDropdown("StateChoice", {
        Text = "State to force",
        Default = "sprint",
        Values = {"sprint", "walk", "crouch", "slide"},
        Callback = function(v) forceStateChoice = v end,
    })
    StateGroup:AddToggle("ForceOnMove", {
        Text = "Force while moving (old)",
        Default = false,
        Callback = function(v) oldForceStateEnabled = v end,
    })
    StateGroup:AddDropdown("MoveStateSelect", {
        Text = "State on move",
        Default = "Sliding",
    ...
    SaveManager:BuildConfigTable({
        Toggles = { SpeedToggle = true, ForceSprintToggle = true, ESPToggle = true, FullbrightToggle = true },
        Options = { SpeedBonus = true, SprintBonus = true, Multiplier = true, StateChoice = true, MoveStateSelect = true },
    })
    SaveManager:Load()

    print("[Obsidian UI] Loaded. RightShift to toggle.")
else
    -- ==================== FALLBACK MANUAL UI (always works) ====================
    ...
    addSection("STATE FORCING (NEW)")
    addToggle("Force Sprint Always", false, function(v)
        forceSprintEnabled = v
        if v then
            LocalPlayer:SetAttribute("StaminaVuota", false)
        end
    end)
    addDropdown("State to force", {"sprint", "walk", "crouch", "slide"}, "sprint", function(v) forceStateChoice = v end)

    addSection("OLD STATE FORCING (on move)")
    addToggle("Force on Move", false, function(v) oldForceStateEnabled = v end)
    addDropdown("State on move", {"Sliding", "Sprinting", "Walking", "Crouching", "Falling"}, "Sliding", function(v) forcedState = v end)
    ...
    print("[Manual UI] Loaded. RightShift to toggle.")
end